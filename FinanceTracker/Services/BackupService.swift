...
class BackupService {
    ...
    func createBackup(inDirectory backupDirectory: URL? = nil) -> URL? {
        ...
        do {
            ...
            let fileName = "FinanceTracker_backup_\(timestamp).csv"
            
            let backupURL = targetDirectory.appendingPathComponent(fileName)
            
            // 使用CSV格式导出（可被Excel打开）
            if CSVProcessor.exportToCSV(backupData: backupData, to: backupURL) {
                return backupURL
            } else {
                return nil
            }
    ...
    func restoreFromBackup(backupURL: URL) -> Bool {
        do {
            // 尝试使用CSV处理器导入数据
            if let backupData = CSVProcessor.importFromCSV(fileURL: backupURL) {
                // 使用解析的数据恢复
                return restoreFromBackupData(backupData)
            } else {
                // 如果CSV导入失败，尝试作为JSON文件处理（向后兼容）
                let data: Data
    ...
    /// - Returns: 是否成功恢复
    private func restoreFromBackupData(_ backupData: BackupData) -> Bool {
        // 替换现有数据
    ...
    func getAvailableBackups(inDirectory directory: URL? = nil) -> [URL] {
        ...
        do {
            ...
            // 查找.csv和.json扩展名的文件（向后兼容）
            let backupFiles = fileURLs.filter { 
                ($0.pathExtension == "csv" || $0.pathExtension == "json") && 
                $0.lastPathComponent.contains("FinanceTracker_backup_") 
            }
